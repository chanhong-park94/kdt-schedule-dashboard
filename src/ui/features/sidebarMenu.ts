...
// IA 정리: 학사일정(timeline)·훈련생분석(analytics)·훈련생이력(traineeHistory)·
// 문의응대(inquiry)·만족도(satisfaction)·교차분석(crossAnalysis) 6개 탭을 사이드바에서 제거.
// (타입/모듈/패널은 유지 — 메뉴에서만 숨김, cross-link은 계속 동작)
...
export function loadSidebarMenuConfig(): SidebarMenuConfig {
  ...
  try {
    ...
    const labels = {
      ...
      instructor: normalizeSidebarMenuLabel(
        "instructor",
    ...
    const icons = {
      ...
      instructor: normalizeSidebarMenuIcon(
        "instructor",
