...
const PATCH_NOTES: PatchNote[] = [
  ...
  },
  {
    ...
    date: "2026-05-28",
    items: [
      { tag: "feat", text: "📖 운영지침 — 카테고리 탭형 전환. 사이드바 클릭 시 그 카테고리만 표시, 나머지는 숨김. 한 페이지 세로 스크롤 5,000+px → 평균 400~1,500px로 단축" },
      { tag: "feat", text: "⭐ 즐겨찾기 — 각 카드 좌상단 별 아이콘으로 추가/해제. 사이드바 맨 위 '내 즐겨찾기' 가상 카테고리에 자동 모임. localStorage 영구 저장 (kdt_guideline_favorites_v1)" },
      { tag: "feat", text: "📝 인라인 메모 — 카드 펼치면 본문·태그 아래 textarea 노출. 입력 후 500ms debounce 자동 저장 + '✓ 저장됨' 인디케이터. localStorage 영구 저장 (kdt_guideline_notes_v1)" },
      { tag: "feat", text: "출처 필터 칩 — 검색창 아래 '운영지침 / 내배카 규정 / 직능 규정' 3개 칩으로 표시 토글. 법령 부분 안 볼 때 사이드바·검색결과에서 모두 제외. 활성 카테고리가 사라지면 첫 가용 카테고리로 자동 fallback" },
      { tag: "improve", text: "검색 동작 분리 — 검색어 있을 때는 출처 필터를 통과한 전 카테고리 가로질러 매칭, 검색어 없을 때는 활성 카테고리 한 개만 렌더 (인지 부하 최소)" },
      { tag: "improve", text: "카드 헤더 구조 — 즐겨찾기 별 버튼 + 제목/펼침 영역 분리. 별 클릭이 카드 펼침으로 잘못 전파되지 않음" },
      { tag: "improve", text: "포커스 이동 — Alt+G 빠른 검색 결과 점프 시 해당 카드의 출처가 OFF였다면 자동 ON 처리 (필터 때문에 점프 후 안 보이는 회귀 방지)" },
    ],
  },
  {
    version: "v3.8.0",
    date: "2026-05-26",
    items: [
      { tag: "feat", text: "📖 26년도 KDT 운영지침 매뉴얼 페이지 신설 — 사이드바 신규 탭 '참고자료' 그룹. 12개 카테고리(사업개요/과정선정/위탁계약/변경/선발/실시/출결/지급/보고/감독/단기/부록) × 73개 항목 (PDF 22p 전체 구조화)" },
      { tag: "feat", text: "운영지침 검색 — 텍스트 매칭 + 노란색 <mark> 하이라이트(200ms debounce) + 추천 검색어 12종(출석률/훈련장려금/재해보험/제적/특별훈련수당/취업률/수료/선지급/자료보관/프로젝트/수강철회 등)" },
      { tag: "feat", text: "Alt+G 전역 단축키 — 어디서나 빠른 검색 모달 호출(↑↓ 이동·Enter 점프·Esc 닫기), 최대 10건 결과 카드 + 추천 검색어 chip" },
      { tag: "feat", text: "운영규정 2개 카테고리 추가 (2026.5.11 개정) — ⚖️ 내일배움카드 운영규정 25개 조항(§3 적용범위~§59 회계감사) + 📜 현장 실무인재 양성 운영규정 제3장 KDT 전용 15개 조항(§14 요건~§29 준용)" },
      { tag: "feat", text: "카드 시각 분류 — 빨간 좌측 보더(critical: 제적/부정훈련/온라인 유의·재해보험·자료5년 보관·훈련기관 의무·선도기업 연대책임), 파란 좌측 보더(info: 자부담·장려금·특별수당·취업률 산정·수료 정의 등)" },
      { tag: "feat", text: "원문 페이지·조문 참조 표시 — 각 카드 우상단에 p.24 / 내배카 §34 / 직능 §14 등 분쟁 시 원문 확인 근거 제공" },
      { tag: "improve", text: "사이드바 메뉴 v6→v7 — '참고자료' 섹션 신설(guideline 1종). 사용자 정의 라벨/아이콘은 신규 키 'guideline'만 default 적용(1회 자동 리셋)" },
      { tag: "improve", text: "마크다운 굵은 글씨 지원 — 운영규정 본문의 **bold** 표기 → <strong> 자동 변환(highlight 유틸 확장)" },
      { tag: "improve", text: "업무 가이드 — '운영지침' 탭 가이드 신설(검색·카테고리·단축키 사용법), '강사 대시보드' 가이드 추가 신설(v3.6 IA 통합 후 누락 보완), 전체 가이드(OVERVIEW)에 Alt+G·매뉴얼 안내 추가" },
    ],
  },
  {
    version: "v3.7.0",
    date: "2026-05-15",
    items: [
      { tag: "feat", text: "📊 하차방어율 주차별 트래커 신설 — 운영 중 기수별로 주차 단위 위험군/이탈 추이를 표·차트로 시각화. 엑셀 스냅샷 업로드 시 자동 파싱·임포트" },
      { tag: "feat", text: "하차방어율 트래커 — 주차별 만족도 연관성 표시 + 학생별 액션 즉시 입력(메모) 칸 추가" },
      { tag: "feat", text: "출결현황 기수 dropdown — 개강 전(startDate > 오늘) 기수를 자동 제외하여 선택 실수 방지" },
      { tag: "improve", text: "하차방어율 페이지 정보 구조 — 운영 지표 카드를 상단, 업데이트 분석 섹션을 하단으로 재배치(중요 지표 즉시 노출)" },
      { tag: "improve", text: "하차방어율 진입 시 자동 조회 제거 — '조회' 버튼 클릭 시에만 fetch (의도치 않은 비용 차단)" },
      { tag: "improve", text: "사이드바 섹션 라벨 통일 — '강사' → '교육사업관리'(IA Phase 1 통합 탭 명칭과 일치)" },
      { tag: "fix", text: "[Slack] 자동 알림 scheduler 부팅 가드를 인증 세션 기준으로 교체 — admin-mode 클래스가 늦게 적용되는 사례(Google JWT 세션은 있는데 body.admin-mode 미활성)에서도 정상 부팅" },
      { tag: "fix", text: "[Slack] 모든 슬랙 알림 필드 변경 즉시 localStorage 자동 저장 — 입력 후 새로고침 시 사라지던 광범위 회귀 해소" },
      { tag: "fix", text: "[Slack] 자동 알림 on/off 토글 변경 시 즉시 localStorage 영구화 — 탭 닫기/새로고침에도 토글 상태 유지" },
    ],
  },
  {
    version: "v3.6.0",
