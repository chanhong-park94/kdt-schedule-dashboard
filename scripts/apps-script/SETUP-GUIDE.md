...
**경우 A — 신청서/증빙자료가 별도 스프레드시트:** 각 응답 시트에서 별도로 설정 (총 2회), `FORM_TYPE` 을 각각 `application` / `evidence` 로 지정.

**경우 B — 한 스프레드시트에 두 폼이 함께 들어오는 통합 응답 시트** (예: "공가 신청 증빙자료 제출_…(응답)" 에 `설문지 응답 시트1`=증빙자료, `설문지 응답 시트2`=신청서):
**1회만** 설정하면 된다. `FORM_TYPE` 을 `auto` 로 두면 제출된 폼의 컬럼 수(신청서 7 / 증빙자료 5)로 자동 판별한다. 트리거 1개가 두 폼을 모두 처리한다.
